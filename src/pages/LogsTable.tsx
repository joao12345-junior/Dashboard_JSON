...
export function LogsTable({ onNavigate }: LogsTableProps) {
	...
	// Detecta o tipo predominante dos logs para buscar as colunas corretas.
	// useMemo evita recalcular isso a cada render — só recalcula quando logs muda.
	const columns = useMemo((): ColumnDefinition[] => {
		if (logs.length === 0) return [];
		// Usa o primeiro log como amostra para detectar o tipo
		const sampleLog = logs[0];
		// detectLogType precisa do formato bruto — aqui usamos logType que já está no Log
		const mapper = getMapper(sampleLog.logType);
